...
--  · points.sql 먼저 실행돼 있어야 함 (stores.points_on/points_rate, orders.points_awarded)
--  · Supabase SQL Editor 내용을 전부 지우고(⌘A → 삭제) 이 파일만 붙여넣고 RUN.
--  · 함수마다 고유 구분자($apply$ 등) 사용 → 다른 SQL이 섞여도 안전. 재실행 안전.
-- ════════════════════════════════════════
...
returns integer language plpgsql security definer set search_path = public as $apply$
...
-- 주문 적립 동기화 (완료→적립 / 완료이탈→회수). 내역도 기록
create or replace function sync_order_points(p_order uuid)
returns integer language plpgsql security definer set search_path = public as $sync$
...
language sql security definer set search_path = public as $list$
...
language sql security definer set search_path = public as $log$
...
returns integer language plpgsql security definer set search_path = public as $adj$
...
begin
  ...
  if p_amount = 0 then
    select points into v_new from customers where id = p_customer;
    return v_new;
  end if;
  v_new := _apply_points(p_customer, p_amount,
    case when p_amount > 0 then 'admin_add' else 'admin_sub' end,
    coalesce(nullif(trim(p_memo), ''), case when p_amount > 0 then '관리자 지급' else '관리자 차감' end), null);
  return v_new;
end;
$adj$;
grant execute on function admin_adjust_points(uuid, int, text) to authenticated;

-- 손님: 결제 시 적립금 사용 (세션 토큰 확인, 잔액 부족이면 0). 반환 = 실제 사용액
create or replace function customer_use_points(p_token text, p_amount int, p_order uuid)
returns integer language plpgsql security definer set search_path = public as $use$
